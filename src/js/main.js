...
ear.svg(document.querySelector("#origami-2"), (svg) => {
    ...
    const grayLine1 = bottom.line()
        .stroke("lightgray")
        .addClass("stroke-pen-1")
        .strokeDasharray("7 14");
    const grayLine2 = bottom.line()
        .stroke("lightgray")
        .addClass("stroke-pen-1")
        .strokeDasharray("7 14");
    const grayLine3 = bottom.line()
        .stroke("lightgray")
        .addClass("stroke-pen-1")
        .strokeDasharray("7 14");
    const grayDots = [
    ...
    const curve = bottom.path()
        .stroke("gold")
        .addClass("stroke-gold")
        .fill("none");
    const midLine = top.line()
        .stroke("blue")
        .addClass("stroke-blue");
    const midPoints = [
...
// grid lines
    ...
    gridLayer.line(0, -GRIDS, 0, GRIDS)
        .stroke("darkgray")
        .setClass("stroke-pen-1");
    gridLayer.line(-GRIDS, 0, GRIDS, 0)
        .stroke("darkgray")
        .setClass("stroke-pen-1");
    ...
    const update = (mouse) => {
        ...
        if (mouse.buttons) {
            drawLayer.circle(mouse.press, 0.2)
                .fill("black")
                .setClass("fill-pen-0");
        }
...
ear.svg(document.querySelector("#blank-test"), (svg) => {

});

const origamiElements = document.getElementsByClassName('tab-content');

for (let i = 0; i < origamiElements.length; i++) {
    origamiElements[i].addEventListener('touchstart', function(event) {
        event.preventDefault();
    }, { passive: false });

    origamiElements[i].addEventListener('touchmove', function(event) {
        event.preventDefault();
    }, { passive: false });
}